...
- `.github/configs/`, `.agents/skills/`: 将来追加される任意のディレクトリに自動対応
...
- ディレクトリ名が `.{claude,codex,agents,cursor,aider,codeium,copilot}` のパターンに一致
...
- **skills/ の構造維持**: `.agents/skills` へのシンボリックリンクを優先し、
  シンボリックリンクが使えない環境では
...
- `AGENTS.md`, `AGENTS.md`: セクション構造をテンプレートに合わせてリファクタリング提案
...
- 既存のskillsとhooks
- `.githooks/` の構造と運用有無
- `.claude/` `.codex/` `.agents/` のトップレベル構造（設定ファイルとsymlink）
...
- ドキュメントファイルの更新（`AGENTS.md`, `AGENTS.md`）
