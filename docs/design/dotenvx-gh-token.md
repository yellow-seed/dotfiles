...
- `.env` を生成するだけでは shell の環境変数にはならないため、Remote では shell 起動時に source するか、`scripts/gh-remote` で source して使う
...
- Remote の AI エージェントは、生成済み `.env` を source した shell または `scripts/gh-remote` 経由で `gh` を実行する
...
`scripts/setup-remote-env` は、`.env.remote` を復号して AI 用の値だけからなる Git 管理されない `.env` を生成する。
実装は次の方針にする。
...
- `scripts/gh-remote` で `.env` を source してから `gh "$@"` を実行する

`gh` 直実行を自然に使いたい場合は、Remote shell の初期化で `.env` を source する。
shell 初期化を制御しづらい場合は、`scripts/gh-remote` を使う。
...
3. AI エージェント向けの環境変数ポリシーを追加する
   ...
   - Remote shell で `.env` を source する、または `scripts/gh-remote` で source する
   - Remote の起動セットアップ用に `scripts/setup-remote-env` を追加する
   - Remote の `gh` 操作用に `scripts/gh-remote` を追加する
...
`scripts/gh-remote` でも同じ結果になることを期待する。
...
| AI プロセスが `gh` を使えなくなる                               | Remote 起動時に `.env.remote` から `.env` を生成し、shell または `scripts/gh-remote` で source する |
| `.env` が生成されただけで shell に反映されない                  | shell 初期化で source するか、`scripts/gh-remote` で source する                                    |
...
- Remote では shell 初期化または `scripts/gh-remote` で `.env` を source して `gh` を実行できる
